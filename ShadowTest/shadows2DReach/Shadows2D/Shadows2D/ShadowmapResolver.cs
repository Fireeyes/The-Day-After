...
namespace Shadows2D
{
    ...
    class ShadowmapResolver
    {
        ...
        public void LoadContent(ContentManager content)
        {
            ...
            blender = content.Load<Effect>("2xMultiBlend");

            SurfaceFormat surfaceFormat = SurfaceFormat.Color;

            distortRT = new RenderTarget2D(graphicsDevice, baseSize, baseSize, false, surfaceFormat, DepthFormat.None);
            distancesRT = new RenderTarget2D(graphicsDevice, baseSize, baseSize, false, surfaceFormat, DepthFormat.None);
            shadowMap = new RenderTarget2D(graphicsDevice, 2, baseSize, false, surfaceFormat, DepthFormat.None);
            reductionRT = new RenderTarget2D[reductionChainCount];
            for (int i = 0; i < reductionChainCount; i++)
            {
                reductionRT[i] = new RenderTarget2D(graphicsDevice, 2 << i, baseSize, false, surfaceFormat, DepthFormat.None);
            }
